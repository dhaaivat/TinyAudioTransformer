...
class MiniSpeechCommandsMelDataset(Dataset):
    def __init__(self, root_dir=DATASET_DIR, split="train", sample_rate=16000, mel_bins=128, augment=False):
        ...
        if self.augment:
            self.waveform_augment = get_waveform_augmentations(sample_rate=sample_rate)
        else:
    ...
    def __getitem__(self, idx):
        ...
        mel_spec = pad_or_trim(mel_spec, max_frames=64)

        return mel_spec, label

def get_dataloaders(batch_size=32, use_augmentation=False):
    train_set = MiniSpeechCommandsMelDataset(split="train", augment=use_augmentation)
